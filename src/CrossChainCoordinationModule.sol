...
interface IAgentCrossChain {
    ...
    function initiateCrossChainCoordination(bytes32, CrossChainConfig calldata, bytes calldata)
        external
        payable
        returns (bytes32);
...
contract CrossChainCoordinationModule is IAgentCrossChain {
    function initiateCrossChainCoordination(bytes32 id, CrossChainConfig calldata, bytes calldata)
        external
        payable
        returns (bytes32)
    {
    ...
    function finalizeCoordination(bytes32) external pure returns (bool) {
        return true;
    }
