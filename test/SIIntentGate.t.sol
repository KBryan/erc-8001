...
contract SIIntentGateTest is Test {
    ...
    function _build(bytes32 kidR, uint64 nonce, uint64 ttl, bytes32 mpHash, bytes32 ctHash, address agent)
        internal
        pure
        returns (SIIntentLib.Header memory h)
    {
        h.dom = DOM;
        h.agent = agent;
        h.kidR = kidR;
    ...
    function test_accept_valid_EOA() public {
        SIIntentLib.Header memory h =
            _build(0x0, 1, uint64(block.timestamp + 600), keccak256("mp"), keccak256("ct"), alice);
    ...
    function test_reject_expired() public {
        SIIntentLib.Header memory h =
            _build(0x0, 1, uint64(block.timestamp - 1), keccak256("mp"), keccak256("ct"), alice);
        bytes memory sig = _sign(alice, aliceKey, h);
        SIIntentGate.SubmitParams memory p = SIIntentGate.SubmitParams({header: h, signature: sig});
    ...
    function test_replay_protection() public {
        SIIntentLib.Header memory h =
            _build(0x0, 42, uint64(block.timestamp + 600), keccak256("mp"), keccak256("ct"), alice);
        bytes memory sig = _sign(alice, aliceKey, h);
        SIIntentGate.SubmitParams memory p = SIIntentGate.SubmitParams({header: h, signature: sig});
    ...
    function test_domain_separation() public {
        // Sign for gate A
        SIIntentLib.Header memory h =
            _build(0x0, 7, uint64(block.timestamp + 600), keccak256("mp"), keccak256("ct"), alice);
        bytes memory sig = _sign(alice, aliceKey, h);
    ...
    function test_kidR_binding() public {
        ...
        SIIntentLib.Header memory h =
            _build(bytes32(uint256(1)), 1, uint64(block.timestamp + 600), keccak256("mp"), keccak256("ct"), alice);
    ...
    function test_nonce_scoped_per_agent() public {
        ...
        SIIntentLib.Header memory h1 =
            _build(0x0, 5, uint64(block.timestamp + 600), keccak256("mp1"), keccak256("ct1"), alice);
        bytes memory s1 = _sign(alice, aliceKey, h1);
        gate.acceptIntent(SIIntentGate.SubmitParams({header: h1, signature: s1}));

        SIIntentLib.Header memory h2 =
            _build(0x0, 5, uint64(block.timestamp + 600), keccak256("mp2"), keccak256("ct2"), bob);
    ...
    function test_malleable_s_rejected() public {
        SIIntentLib.Header memory h =
            _build(0x0, 9, uint64(block.timestamp + 600), keccak256("mp"), keccak256("ct"), alice);
        bytes32 dig = SIIntentLib.digest(gate.DOMAIN_SEPARATOR(), h);
    ...
    function test_mpHash_binding() public {
        SIIntentLib.Header memory h =
            _build(0x0, 10, uint64(block.timestamp + 600), keccak256("mpA"), keccak256("ctA"), alice);
        bytes memory sig = _sign(alice, aliceKey, h);
    ...
    function test_erc1271_signature() public {
        ...
        SIIntentLib.Header memory h =
            _build(0x0, 11, uint64(block.timestamp + 600), keccak256("mp"), keccak256("ct"), address(wallet));
        bytes32 dig = SIIntentLib.digest(gate.DOMAIN_SEPARATOR(), h);
